  ...
  connect(pool.options, (err, connection) => {
    ...
    if (err) {
      ...
      if (pool.options.legacyCompatMode === false) {
        // we want to emit the error so its stored on the topology description if running
        // in the unified topology, but only while the pool is connecting
        if (pool.state === CONNECTING) {
          pool.emit('error', err);
        }
      }
